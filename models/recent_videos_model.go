...
func (d itemVideoDelegate) Render(w io.Writer, m list.Model, index int, listItem list.Item) {
	...
	str := fmt.Sprintf("%d. %s %s %s", index+1, i.video.Author, i.video.Title, utils.FormatRelativeTime(i.video.PublishedDate))

	fn := d.styles.ListItemStyle.CardStyle.Render
	if index == m.Index() {
		fn = func(s ...string) string {
			selectedStyle := d.styles.ListItemStyle.SelectedStyle.Width(m.Width())
